...
const[country,setCountry]=useState(match.params.CountryId)
...
setfetchedCities('')
  ...
  useEffect(async () => {
    const  getArea=async(id)=>{
      ...
      try {
        const responsee = await fetch(
          `${global.apiUrl}/super/areas?city=${match.params.CityId}&paginate=0`,
          {
            method: "GET",
            headers: {
              Authorization: "Bearer " + userToken,
        ...
        if(response.success==true){
          setUpData({
            ...
            city_id: response.payload.filter(item=>item.id==id)[0].city_id,
         })
  ...
  const handleUpdate= async (e) => {
    ...
    try {
      const responsee = await fetch(
        `${global.apiUrl}/super/areas/${match.params.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: "Bearer " + userToken,
      ...
      if (response.success) {
        await setVisible(6)
        setSuccessAdd(i18n.language == 'ar' ? "تم تعديل منطقة بنجاح" : "Area Updated Successfuly")
